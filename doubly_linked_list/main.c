...
void dll_displayMenu();
void dll_processAddBook(BookList *books);
void dll_processAddBorrower(Queue *borrowers, Stack *history);
void dll_processBorrowing(BookList *books, Queue *borrowers, Stack *history);
void dll_processCancelLastRequest(Stack *history);
void dll_processReturnBook(BookList *books, Stack *history);
...
int dll_main() {
    ...
    dll_initBookList(&bookList);
    initQueue(&borrowersQueue);
    initStack(&history);
    
    int choice;
    
    printf("\n===== Perpustakaan di Perguruan Tinggi (Implementasi Doubly Linked List) =====\n");
    ...
    do {
        ...
        switch (choice) {
            case 1:
                dll_processAddBook(&bookList);
                break;
            case 2:
                dll_processAddBorrower(&borrowersQueue, &history);
                break;
            case 3:
                dll_processBorrowing(&bookList, &borrowersQueue, &history);
                break;
            case 4:
                dll_processCancelLastRequest(&history);
                break;
            case 5:
                dll_processReturnBook(&bookList, &history);
                break;
            case 6:
                dll_displayBooks(&bookList);
                break;
            case 7:
                dll_displayBooksReverse(&bookList);
                break;
            case 8:
...
void dll_processBorrowing(BookList *books, Queue *borrowers, Stack *history) {
    ...
    BookNode *book = dll_findBook(books, bookName);
    if (book == NULL) {
...
int dll_verifyBorrower(Stack *history, const char *bookName, const char *borrowerName) {
...
void dll_processReturnBook(BookList *books, Stack *history) {
    ...
    if (!dll_verifyBorrower(history, bookName, borrowerName)) {
